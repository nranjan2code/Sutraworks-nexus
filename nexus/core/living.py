...
class LivingNEXUS(nn.Module):
    ...
    def interact(
        ...
        if not responded:
            ...
            refusal_logits = self.refusal_generator(batch_size, seq_len, logits.device)
...
def create_living_nexus(
    ...
    **config_overrides,
    ...
    if architecture == "flowing":
        ...
        model = create_flowing_nexus(size=size, **model_kwargs)
    else:
        # Traditional layered architecture
        from nexus.core.nexus_core import create_nexus_model

        model = create_nexus_model(size=size, **model_kwargs)
