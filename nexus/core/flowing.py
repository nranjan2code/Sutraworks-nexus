...
class DynamicsDivergenceError(Exception):
    """Raised when flowing dynamics diverge (NaN or Inf encountered)."""
    ...
    def forward(
        ...
        for step in range(self.config.max_flow_steps):
            ...
            if torch.isnan(z_new).any() or torch.isinf(z_new).any():
                raise DynamicsDivergenceError(
                    f"Flow dynamics diverged at step {step}. " f"Energy: {energy.item():.4f}"
                )
